# Copied some parts from  The InstructPix2Pix Authors and The HuggingFace Team under Apache2.0 License
#
...
def preprocess(image):
    ...
    elif isinstance(image[0], jt.Var):
        image = jt.concat(image, dim=0)
    return image


# Copied from diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_img2img.retrieve_latents
def retrieve_latents(
    encoder_output: jt.Var, sample_mode: str = "sample"
):
...
class StableDiffusionInstructPix2PixPipeline(
        ...
        **kwargs,
        ...
        if ip_adapter_image is not None:
            ...
            if self.do_classifier_free_guidance:
                image_embeds = jt.concat([image_embeds, negative_image_embeds, negative_image_embeds])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = jt.concat([latents] * 3) if self.do_classifier_free_guidance else latents

                # concat latents, image_latents in the channel dimension
                scaled_latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                scaled_latent_model_input = jt.concat([scaled_latent_model_input, image_latents], dim=1)
    ...
    def _encode_prompt(
        ...
        if do_classifier_free_guidance:
            ...
            prompt_embeds = jt.concat([prompt_embeds, negative_prompt_embeds, negative_prompt_embeds])
    ...
    def prepare_image_latents(
        self, image, batch_size, num_images_per_prompt, dtype, do_classifier_free_guidance,
    ):
        if not isinstance(image, (jt.Var, PIL.Image.Image, list)):
            raise ValueError(
                f"`image` has to be of type `jt.Var`, `PIL.Image.Image` or list but is {type(image)}"
            )
        ...
        if batch_size > image_latents.shape[0] and batch_size % image_latents.shape[0] == 0:
            ...
            image_latents = jt.concat([image_latents] * additional_image_per_prompt, dim=0)
        elif batch_size > image_latents.shape[0] and batch_size % image_latents.shape[0] != 0:
            raise ValueError(
                f"Cannot duplicate `image` of batch size {image_latents.shape[0]} to {batch_size} text prompts."
            )
        else:
            image_latents = jt.concat([image_latents], dim=0)

        if do_classifier_free_guidance:
            uncond_image_latents = jt.zeros_like(image_latents)
            image_latents = jt.concat([image_latents, image_latents, uncond_image_latents], dim=0)
