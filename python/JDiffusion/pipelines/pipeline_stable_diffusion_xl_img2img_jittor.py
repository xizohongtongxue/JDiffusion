...
from JDiffusion.pipelines.pipeline_output_jittor import StableDiffusionXLPipelineOutput
...
class StableDiffusionXLImg2ImgPipeline(
    ...
    def encode_prompt(
        ...
        pooled_prompt_embeds: Optional[jt.Var] = None,
        negative_pooled_prompt_embeds: Optional[jt.Var] = None,
        ...
        Args:
            ...
            negative_prompt_embeds (`jt.Var`, *optional*):
                ...
                argument.
            pooled_prompt_embeds (`jt.Var`, *optional*):
                Pre-generated pooled text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting.
                If not provided, pooled text embeddings will be generated from `prompt` input argument.
            negative_pooled_prompt_embeds (`jt.Var`, *optional*):
        ...
        if prompt_embeds is None:
            ...
            prompt_embeds = jt.concat(prompt_embeds_list, dim=-1)

        # get unconditional embeddings for classifier free guidance
        zero_out_negative_prompt = negative_prompt is None and self.config.force_zeros_for_empty_prompt
        if do_classifier_free_guidance and negative_prompt_embeds is None and zero_out_negative_prompt:
            negative_prompt_embeds =jt.zeros_like(prompt_embeds)
            negative_pooled_prompt_embeds = jt.zeros_like(pooled_prompt_embeds)
            ...
            negative_prompt_embeds = jt.concat(negative_prompt_embeds_list, dim=-1)
    ...
    def encode_image(self, image,  num_images_per_prompt, output_hidden_states=None):
        ...
        if output_hidden_states:
            ...
            uncond_image_enc_hidden_states = self.image_encoder(
                jt.zeros_like(image), output_hidden_states=True
            ).hidden_states[-2]
            ...
            uncond_image_embeds = jt.zeros_like(image_embeds)
    ...
    def _get_add_time_ids(
        ...
        add_time_ids = jt.array([add_time_ids], dtype=dtype)
        add_neg_time_ids = jt.array([add_neg_time_ids], dtype=dtype)
    ...
    def get_guidance_scale_embedding(self, w, embedding_dim=512, dtype=jt.float32):
        """
        ...
        emb = jt.log(jt.array(10000.0)) / (half_dim - 1)
        emb = jt.exp(jt.arange(half_dim, dtype=dtype) * -emb)
        ...
        **kwargs,
        ...
        Args:
            ...
            prompt_2 (`str` or `List[str]`, *optional*):
                The prompt or prompts to be sent to the `tokenizer_2` and `text_encoder_2`. If not defined, `prompt` is
                used in both text-encoders
            image (`jt.Var` or `PIL.Image.Image` or `np.ndarray` or `List[jt.Var]` or `List[PIL.Image.Image]` or `List[np.ndarray]`):
            ...
            seed (`int` or `List[int]`, *optional*):
                One or a list of [seed(s)] to make generation deterministic.
        ...
        if self.do_classifier_free_guidance:
            prompt_embeds = jt.concat([negative_prompt_embeds, prompt_embeds], dim=0)
            add_text_embeds = jt.concat([negative_pooled_prompt_embeds, add_text_embeds], dim=0)
            add_neg_time_ids = add_neg_time_ids.repeat(batch_size * num_images_per_prompt, 1)
            add_time_ids = jt.concat([add_neg_time_ids, add_time_ids], dim=0)
        ...
        if ip_adapter_image is not None:
            ...
            image_embeds, negative_image_embeds = self.encode_image(
                ip_adapter_image,  num_images_per_prompt, output_hidden_state
            )
            if self.do_classifier_free_guidance:
                image_embeds = jt.concat([negative_image_embeds, image_embeds])
        ...
        if not output_type == "latent":
            # make sure the VAE is in float32 mode, as it overflows in float16
            needs_upcasting = self.vae.dtype == jt.float16 and self.vae.config.force_upcast
